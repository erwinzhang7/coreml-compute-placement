...
`MLComputePlan`, cost-weighted, for the two SigLIP variants. PAPER.md §3.2 shows that the
default can be slower than both explicit placements but deliberately does not claim the
mechanism, because the partition was only instrumented here, for one architecture:
...
| M4 Pro | naive | **78.9% ANE / 21.1% GPU** | **172.2** | 204.4 (ANE) | **below both** |
| M4 Pro | ANE-rewritten | ~98.5% ANE | 240.9 | 231.3 (ANE) | slightly better |

The M4 Pro naive row is the one that matters: `ALL` splits the graph 79/21 and that split runs
slower than pure ANE (204.4) *and* slower than pure GPU (178.8). Cross-device handoff costs
more than the parallelism gains. Where `ALL` resolves to a single unit it matches that unit,
so the cost appears only when it actually splits.

These four rows have **no backing JSON in `results/`**; they were measured by hand and
`run.sh` runs `anecheck.py` on the naive model only. Treat them as weaker evidence than
anything with a file behind it until that is fixed.
